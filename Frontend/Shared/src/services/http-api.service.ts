...
import { CustomError, ErrorCode, LoginResponse, SMap, User } from '@shared/models';
import { AuthState, AuthStore } from '@shared/storage';
...
export class HttpApiService {
    ...
    async refreshToken(url?: string, force?: boolean): Promise<CustomError> {
        const [token, refreshToken, user] = await Promise.all([
            this.authStore.getFromStorage<string>('token'),
            this.authStore.getFromStorage<string>('refresh_token'),
            this.authStore.getFromStorage<User>('user'),
        ]);
        ...
        const date: Date = await this.authStore.getFromStorage('expires');
        ...
        const return_result = resp instanceof CustomError && refreshToken === (await this.authStore.getFromStorage<string>('refresh_token')) ? resp : null;
