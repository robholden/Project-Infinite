...
export class AuthService {
    constructor(private api: HttpApiService, private state: AuthState, private sockets: SocketService) {}
    ...
    async login(username: string, password: string, apiOpts?: ApiOptions<LoginResponse>): Promise<LoginResponse | CustomError> {
        ...
        // Update temp
        this.state.set('savedUsername', username);
    ...
    async enable2FA(type: TwoFactorType, code: string): Promise<RecoveryCode[] | CustomError> {
        ...
        this.state.update('user', (user) => ({ ...user, twoFactorEnabled: true, twoFactorType: type }));
    ...
    async disable2FA(passwordRequest: PasswordRequest): Promise<void | CustomError> {
        ...
        this.state.update('user', (user) => ({ ...user, twoFactorEnabled: false, twoFactorType: TwoFactorType.Unset }));
    }
