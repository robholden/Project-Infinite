...
export class SocketService {
    ...
    constructor(@Inject(INJ_ENV) private env: Environment, private authState: AuthState, private http: HttpApiService) {}

    async connect() {
        const [token, authToken] = await Promise.all([this.authState.retrieve('token'), this.authState.retrieve('refreshToken')]);
        ...
        const conn = new signalR.HubConnectionBuilder()
            .withUrl(endpoint, {
                accessTokenFactory: async () => {
                    ...
                    return await this.authState.retrieve('token');
                },
