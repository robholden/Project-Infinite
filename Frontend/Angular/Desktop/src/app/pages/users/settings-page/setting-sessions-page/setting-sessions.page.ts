...
export class SettingSessionsPage implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.sockets.on('SessionRevoked', 'sessions_page', async (authToken) => {
            if (!authToken || authToken === this.authState.snapshot('authToken')) return;
            this.get();
        });
    }
    ...
    private async get() {
        this.authToken = this.authState.snapshot('authToken');
    ...
    async triggerDelete(index: number = null) {
        ...
        if (this._pw || !!this.authState.snapshot('provider')) {
